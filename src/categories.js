...
export const findCategoryLetter = async (productType) => {
  try {
    ...
    for (let i = 0; i < categories.length; i++) {
      if (categories[i].key === productType) return categories[i].letter;
    }

    return null;
  } catch (error) {
    console.error("findCategoryLetter error:", error);
    return null;
  }
};
...
export const addCategory = async (inputParams) => {
  try {
    ...
    if (!/^[A-Z]$/.test(upperLetter)) return { success: false, message: "Letter must be a single letter A-Z" };

    for (let i = 0; i < categories.length; i++) {
      if (categories[i].letter === upperLetter) {
        return { success: false, message: `Letter ${upperLetter} is already used by ${categories[i].title}` };
      }
    }
...
export const buildNextItemId = async (productType) => {
  try {
    const letter = await findCategoryLetter(productType);
